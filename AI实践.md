...
## 五、相关子文档引用
为方便深入了解具体执行与过程留痕，以下子文档提供了对应阶段的详细展示与过程说明：
...
- [Prompt/Rule工程实践](./Appendix/prompt-rule工程.md)：Agent角色规范的模板与实践参考（作为技术细节补充，非核心重点）。
...
### Prompt/Rule工程实践
在实践过程中，我们制定了多个可复用的Rule模板，用于规范各Agent的行为与输出。这些Rule模板遵循统一的规范结构（角色定位、目标定义、行为逻辑、风格规范等），可根据不同场景灵活定制。详细内容请参考：[Prompt/Rule工程实践文档](./Appendix/prompt-rule工程.md)。
